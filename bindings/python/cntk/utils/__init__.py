...
from ..cntk_py import NDMask, Value
...
def sanitize_batch(var, batch, seq_starts=None, dtype=None, device=None):
    ...
    if use_mask:
        mask = NDMask((max(seq_lens), len(batch)), device)
    ...
    if sparse.issparse(batch):
        ...
        dim = var.shape[0]
        if dtype == np.float32:
            value = Value.create_one_hot_float(dim, batch, device, False)
        elif dtype == np.float64:
            value = Value.create_one_hot_double(dim, batch, device, False)
    else:
